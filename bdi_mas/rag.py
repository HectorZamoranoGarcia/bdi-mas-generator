"""
rag.py — Módulo RAG (Retrieval-Augmented Generation) para documentación Jason.

Indexa los documentos PDF/TXT/MD/ASL de la carpeta 'docs/' local utilizando
ChromaDB con embeddings ONNX, y expone una herramienta de búsqueda semántica
que los agentes ADK pueden invocar para consultar teoría y sintaxis BDI.
"""
    ...
    def extract_file(f):
        if f.suffix.lower() == ".pdf":
            try:
                ...
                texts.append(f"Aviso: Falta instalar pypdf para procesar {f.name}")
            except Exception:
                pass
...
def build_vectorstore(paths):
    ...
    # Insertar por lotes (ChromaDB limita a 5 461 por batch)
    batch_size = 5000
...
def init_rag(paths=None):
    """Inicializa el vectorstore de forma lazy (singleton)."""
    global VECTORSTORE
    if paths is None:
        paths = DOCS_PATH
...
def search_local_docs(query: str, k: int = 4) -> str:
    ...
    Utiliza esta herramienta cuando tengas dudas sobre cómo programar una
    característica específica en Jason/AgentSpeak o necesites consultar la
    sintaxis teórica y tutoriales.
    ...
    try:
        ...
        if VECTORSTORE.count() == 0:
            return (
                f"Aviso RAG: No se encontraron documentos indexados en "
                f"la carpeta '{DOCS_PATH[0]}' o está vacía."
            )

        result = VECTORSTORE.query(query_texts=[query], n_results=k)

        docs_found = result.get("documents", [])
        if docs_found and docs_found[0]:
            text_blocks = [
                f"--- Fragmento {i+1} ---\n{t}"
                for i, t in enumerate(docs_found[0])
            ]
            return (
                "RESULTADOS DE LA DOCUMENTACIÓN (RAG):\n\n"
                + "\n\n".join(text_blocks)
            )
        else:
            return (
